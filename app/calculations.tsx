...
export const calcWheelInertia = (wheelDia: string, wheelWt: string): string => {
  ...
  const spokeInertia =
    C.SPOKE_MASS_PERCENTAGE *
    (1 / 2) *
    wheelMass_kg *
    (wheelRad_m * wheelRad_m + C.HUB_RADIUS_METERS * C.HUB_RADIUS_METERS); //console.log("spokeInertia is " + spokeInertia)
  const hubInertia =
    wheelDia == '0'
      ? 0
      : C.HUB_MASS_PERCENTAGE *
        (1 / 2) *
        wheelMass_kg *
        C.HUB_RADIUS_METERS *
...
export const calcTotalInertia = (
  ...
  const totalInertia = (parseFloat(tireInertia) + parseFloat(wheelInertia)).toFixed(
    1,
  );
  return totalInertia;
};

export const calcTireInertia = (
  tireDia: string,
  tireWt: string,
  wheelDia: string,
): string => {
  ...
  const sidewallInertia =
    tireDia === '0'
      ? 0
      : C.SIDEWALL_MASS_PERCENTAGE *
        (1 / 2) *
        tireMass_kg *
        (tireRad_m * tireRad_m + wheelRad_m * wheelRad_m); //console.log('sidewallInertia in kg*m^2: ' + sidewallInertia)
  const tireInertia = (treadInertia + sidewallInertia).toFixed(1);
  return tireInertia;
};

export const calcTireDiaFromStdNotation = (
  wheelDia: string,
  aspRat: string,
  tireWd: string,
): string => {
  if (aspRat === '0' || tireWd === '0' || wheelDia === '0') {
    return '0';
  }
