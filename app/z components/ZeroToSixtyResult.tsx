...
const ZeroToSixtyResult: React.FC<ZeroToSixtyResultProps> = (props) => {
  ...
  const mass_alt_car_on_alt_wheels =
    (vehicleWeight -
      4 * (tireWeight1 + wheelWeight1) +
      4 * (tireWeight2 + wheelWeight2)) *
    0.453592;

  const T1 =
    4 * (inertia1 / tireRadius1) + mass_current_car_on_current_wheels * tireRadius1;
  ...
  const secondsFaster: string = (
    currentZeroToSixtyTime -
    (T2 / T1) * currentZeroToSixtyTime
  ).toFixed(2);
  const secondsSlower: string = (
    (currentZeroToSixtyTime - (T2 / T1) * currentZeroToSixtyTime) *
    -1
  ).toFixed(2);
  ...
  let secondsFasterMessage: string =
    '0-60mph | ' + secondsFaster + ' seconds faster';
  let secondsSlowerMessage: string =
    '0-60mph | ' + secondsSlower + ' seconds slower';

  percentDecMessage =
    percentDec !== 'NaN' ? percentDecMessage : 'input more to compare 0-60 %';
  percentIncMessage =
    percentInc !== 'NaN' ? percentIncMessage : 'input more to compare 0-60 %';
  secondsFasterMessage =
    secondsFaster !== 'NaN'
      ? secondsFasterMessage
      : 'input more to compare 0-60 time';
  secondsSlowerMessage =
    secondsSlower !== 'NaN'
      ? secondsSlowerMessage
      : 'input more to compare 0-60 time';
      ...
      {T1 > T2 ? (
        <div>
          ...
          <input
            ...
            disabled
            className="mb-3 h-8 w-11/12 rounded-lg border-none bg-gradient-to-r from-amber-50 to-yellow-200 text-center text-lg font-bold text-green-600"
          />
        </div>
      ) : (
        <div>
          ...
          <input
            ...
            disabled
            className="mb-3 h-8 w-11/12 rounded-lg border-none bg-gradient-to-r from-yellow-300 to-amber-600 text-center text-lg font-bold text-orange-700"
          />
        </div>
      )}
    </>
