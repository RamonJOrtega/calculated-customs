...
const CalcInput: React.FC<CalcInputProps> = (props: CalcInputProps) => {
    ...
    if (typeof props.value === 'boolean') {
        ...
        if (props.title.includes("Inertia")) { inputElement = (<input className='mx-1 rounded-md bg-neutral-800 text-yellow-200 font-bold' type="number" value={props.value} disabled />);} 
        else {inputElement = (<input className='text-white mx-1 w-full rounded-md bg-neutral-700 font-bold' type="number" value={props.value} ref={inputRef} onInput={handleInput} pattern="[0-9.]" placeholder={props.placeHolder} onClick={()=>{props.setValue("")}} list = {compactTitleId} max={maxNums[compactTitleId]} min="0" step = "0.1"/>);}
    }
